...
class WISE(torch.nn.Module):
    def __init__(self, config, model, device):
        ...
        self.distill_logits_history = []  # 存储历史logits用于蒸馏
        self.current_merge_group_logits = []  # 当前merge组的logits
    ...
    def edit(self, config, tokens, act_mask=None, deact_mask=None):
        ...
        for i in range(config.n_iter):
            ...
            # 计算蒸馏损失
            distill_loss = self._cal_distill_loss(current_logits, config)
            ...
            if self.config.retrieve and self.get_adapter_layer().merge_cnt > 0 and self.config.replay:
                print(
                    f"loss {np.round(loss.item(), 3)} = {np.round(ft_loss.item(), 3)} + {np.round(act_loss.item(), 3)} + {np.round(neg_memo_loss.item(), 3)} + {np.round(pos_memo_loss.item(), 3)} + {np.round(distill_loss.item(), 3)}"
                )
            else:
                print(
                    f"loss {np.round(loss.item(), 3)} = {np.round(ft_loss.item(), 3)} + {np.round(act_loss.item(), 3)} + {np.round(distill_loss.item(), 3)}"
                )
        ...
        #     ##wys
        #
        # if getattr(eval(f"self.model.{self.layer}"), "editing_total_cnt") in [0,self.config.merge_freq*1,self.config.merge_freq*2,
        #                                                                       self.config.merge_freq*3]:
        #     print('getattr(eval(f"self.model.{self.layer}"), "editing_total_cnt")',getattr(eval(f"self.model.{self.layer}"), "editing_total_cnt"))
        ...
        #                                     weight_decay=1e-5)
        #
        #     ft_loss, current_logits = self._cal_ft_loss(tokens, last_prompt_token_loc)  # 修改_cal_ft_loss使其返回logits
        #
        #     # 计算蒸馏损失（如果是第一个迭代则不计算）
        #     #distill_loss = 0.0
        #     if self.teacher_logits is not None and current_logits is not None:
        #         T = config.temperature  # 温度系数
        #         if self.teacher_logits.dim() == 1:
        #             self.teacher_logits = self.teacher_logits.unsqueeze(0)  # 添加batch维度
        #         if current_logits.dim() == 1:
        #             current_logits = current_logits.unsqueeze(0)
        #
        #         loss_fn = torch.nn.KLDivLoss(reduction='batchmean')
        #         soft_target = F.softmax(self.teacher_logits / T, dim=-1)
        #         output_log = F.log_softmax(current_logits / T, dim=-1)
        #         distill_loss = loss_fn(output_log, soft_target) * (T ** 2) * config.lamda  # 蒸馏权重
        #
        ...
        #                                          config=config, act_mask=act_mask, deact_mask=deact_mask)
        #     loss = ft_loss + act_loss.to(ft_loss.device) + distill_loss  # 添加蒸馏损失
        #     # train_loss = loss.cpu().detach().numpy()
        #     # plt.subplot(2, 1, 1)  # 2行1列的第1个子图
        #     # plt.plot(train_loss, 'b-o', linewidth=2, markersize=6, label='训练损失')
        #     # plt.title('训练损失曲线', fontsize=14, fontweight='bold')
        #     # plt.ylabel('损失值', fontsize=12)
        #     # plt.xlabel('训练轮次 (Epochs)', fontsize=12)
        #     # plt.grid(True, linestyle='--', alpha=0.7)
        #     # plt.xlim(0, 30 - 1)
        #     # plt.ylim(0, max(train_loss) * 1.1)  # 设置y轴范围
        #     #
        ...
        #     else:
        #         print(
        #             f"loss {np.round(loss.item(), 3)} = {np.round(ft_loss.item(), 3)} + {np.round(act_loss.item(), 3)}+{np.round(distill_loss, 3)}"
        #         )
        #
        #     optimizer.step()
        #     self.loss_meter.update(loss.item())
        #
        #     # 保存当前logits作为下个case的教师logits（在最后一个迭代保存）
        #     if i == config.n_iter - 1 or self.loss_meter.stop():
        #         self.teacher_logits = current_logits.detach().clone()  # 分离梯度
    ...
    def _cal_distill_loss(self, current_logits, config):
        """
        计算蒸馏损失，确保每merge_cnt个数据分布尽可能相似
        """
        if not hasattr(config, 'distill_weight') or config.distill_weight <= 0:
            return torch.tensor(0.0, device=current_logits.device)
        
        distill_loss = torch.tensor(0.0, device=current_logits.device)
        
        # 如果有历史logits，计算与它们的蒸馏损失
        if len(self.distill_logits_history) > 0:
            # 使用最近的几个logits作为教师
            teacher_logits = self.distill_logits_history[-1]  # 使用最近的作为主要教师
            
            # 确保维度匹配
            if teacher_logits.shape != current_logits.shape:
                # 如果形状不匹配，取最后一个token的logits
                teacher_logits = teacher_logits[:, -1, :] if teacher_logits.dim() == 3 else teacher_logits
                current_logits_flat = current_logits[:, -1, :] if current_logits.dim() == 3 else current_logits
            else:
                current_logits_flat = current_logits
            
            # 计算KL散度蒸馏损失
            T = getattr(config, 'temperature', 4.0)  # 温度参数
            alpha = getattr(config, 'distill_alpha', 0.5)  # 蒸馏权重
            
            # 软标签蒸馏
            teacher_soft = F.softmax(teacher_logits / T, dim=-1)
            student_log = F.log_softmax(current_logits_flat / T, dim=-1)
            
            kl_loss = F.kl_div(student_log, teacher_soft, reduction='batchmean') * (T ** 2)
            distill_loss += alpha * kl_loss
            
            # 如果有多个历史logits，计算与它们的平均分布相似性
            if len(self.distill_logits_history) > 1:
                # 计算与历史logits的平均KL散度
                avg_teacher_logits = torch.stack(self.distill_logits_history[-3:]).mean(dim=0)  # 使用最近3个的平均
                if avg_teacher_logits.shape != current_logits.shape:
                    avg_teacher_logits = avg_teacher_logits[:, -1, :] if avg_teacher_logits.dim() == 3 else avg_teacher_logits
                
                avg_teacher_soft = F.softmax(avg_teacher_logits / T, dim=-1)
                avg_kl_loss = F.kl_div(student_log, avg_teacher_soft, reduction='batchmean') * (T ** 2)
                distill_loss += (1 - alpha) * avg_kl_loss
        
        # 将当前logits添加到历史中
        self.current_merge_group_logits.append(current_logits.detach().clone())
        
        return distill_loss * config.distill_weight

    def _manage_distill_logits(self):
        """
        管理蒸馏logits历史，在merge时更新历史记录
        """
        if len(self.current_merge_group_logits) > 0:
            # 计算当前merge组的平均logits
            avg_group_logits = torch.stack(self.current_merge_group_logits).mean(dim=0)
            self.distill_logits_history.append(avg_group_logits)
            
            # 限制历史长度，只保留最近的几个
            max_history = getattr(self.config, 'max_distill_history', 5)
            if len(self.distill_logits_history) > max_history:
                self.distill_logits_history = self.distill_logits_history[-max_history:]
            
            # 清空当前组
            self.current_merge_group_logits = []
            
            print(f"更新蒸馏历史，当前历史长度: {len(self.distill_logits_history)}")
    ...
    def save(self, save_path):
        ...
        additional_info = {
            ...
            'distill_logits_history': self.distill_logits_history,
            'current_merge_group_logits': self.current_merge_group_logits,
    ...
    def load(self, load_path):
        ...
        for key, value in saved_data['additional_info'].items():
            if key in ['distill_logits_history', 'current_merge_group_logits']:
    ...
    def forward(self, *args):
        if self.editing:
            ...
            if not self.config.retrieve:
                ...
                try:
                    self.selected_memory_ID.append(temp[str(min_dist)])  # add by wys
                except:
